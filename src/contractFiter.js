...
const poolMEGA = await obj.get('MEGA', settings.MEGA)
    ...
    let access = await poolMGFT.request().query(
       ...
       ,CASE WHEN (origin IN ('mega-ac-api.azurewebsites.net','mega-ac-api-nunez.azurewebsites.net', 'mega-accesscontrol-bkd.azurewebsites.net', 'mega-accesscontrol-bkd-center2.azurewebsites.net', 'mega-accesscontrol-bkd-nunez.azurewebsites.net')) THEN 'Megatlon'
            ...
            END DBContext
    FROM AccessLogs
    where id between 6090000 AND 6130000
        `
    ...
    for (let i = 0; i < access.length; i++) {
        ...
        if (acc.data_ProcessStep == 'StandardAccessValidationAsyncStep' || acc.data_ProcessStep == 'ProcessingTurnProvider' || acc.data_ProcessStep == 'MigracionCaballitoAsyncStep' || acc.data_ProcessStep == 'ConnusTurnProcessorStep' || acc.data_ProcessStep == 'GetActiveServiceStep'){      
        try {
        ...
        else if (acc.data_ProcessStep == 'GetActiveServiceAsyncStep' || acc.data_ProcessStep == 'ProcessGymPassServicesStep'|| acc.data_ProcessStep == 'GymPassValidatorAsyncStep' || acc.data_ProcessStep == 'GymPassValidatorByHookAsyncStep'|| acc.data_ProcessStep == 'GymPassStep' ){
                try {
