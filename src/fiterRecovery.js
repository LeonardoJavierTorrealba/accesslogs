...
const poolMGFTQA = await poolManager.get('MGFTQA', settings.MGFTQA)
    ...
    // let db = new sqlite3.Database('./database/dbRecuperation/Mega/VillaCrespoReal.db');
...
const serialesSuc = [
    ...
    {idSucursal: 25, idSucursalControlAcceso: "25-58", serial:"21088523701496" },
    {idSucursal: 25, idSucursalControlAcceso: "25-58", serial:"21084523702453" },
    ...
    //     Timestamp as ts
    //     FROM ca
    //     WHERE Timestamp BETWEEN '2022-12-12T20:00:00' AND '2022-12-13T13:00:00')
    // WHERE Action = 'Ready'`
    ...
    // const pool = await connection.getConnectionProd();  
    // pool.connect(); 
//     var selectS = await pool.request().query('SELECT * FROM scans');
//     var scans = selectS.recordset;


    
//     var selectT = await pool.request().query('SELECT * FROM transactions');
    
//     var transactions = selectT.recordset;



let scans = await poolMGFT.query(`select id, partner_id, created, contract_id, origin, product_id from AccessLogs where id between 8589926 AND 8602385 ORDER BY ID DESC`)
scans = scans.recordset;
       
    for (const scan of scans) {      
        
        if(scan.origin == 'fiter-ac-api.azurewebsites.net'){

        let tsScan = moment(scan.created,'YYYY-MM-DD"T"HH:mm:ss.SSS"Z"').format('YYYYMMDD');
        
        
        if(scan.product_id == null || scan.product_id == 'null'){

            var socioConcat = scan.partner_id;       
            let nyphen = socioConcat.indexOf('-');            
         
         let socio = {
         idSucursal: socioConcat.slice(0, nyphen),
         idSocio: socioConcat.slice(nyphen+1)
         }          
        ...
        //  // console.log(request);
         ...
         let resSP = await poolFITER.request().query(`EXEC SP_ServiciosSociosVigencias_Listar ${socio.idSocio}, ${socio.idSucursal},'${tsScan}'`);
         ...
         try{
             ...
             fechaMoment = moment(scan.created,'YYYY-MM-DD"T"HH:mm:ss.SSS"Z"').format('YYYY-MM-DD HH:mm:ss.000')
         }
         catch{               
             vigenciaHasta = 1;
             fechaMoment = 2;
         ...
         if(vigenciaHasta < fechaMoment || resSP == undefined){
             try {
                 let resFunction = await poolFITER.request().query(`SELECT TOP(1) * FROM dbo.FN_Rules_AccessControl_Contrato (${socio.idSocio}, ${socio.idSucursal}, '${tsScan}')`);
            ...
            //  }  
             ...
             poolMGFT.query(`UPDATE accesslogs
                            SET contract_id = '${contractOrService.contract_id}', product_id = '${contractOrService.product_id}'
                            WHERE id = ${scan.id}`)
            ...
            //  poolMGFT.request().query( `INSERT INTO AccessLogs 
            //  VALUES ('${obj.partner_id}','${obj.access_control_external_id}','${obj.device_serial}',${obj.result}, '${obj.message}', 'RecoveryPayload', null, '${obj.origin}', '${obj.app_version}', '${obj.request_id}', '${obj.created}', null, '${contractOrService.contract_id}', '${contractOrService.product_id}')`);     
         
         console.log(`${scan.id} --- ${contractOrService.product_id}`);
